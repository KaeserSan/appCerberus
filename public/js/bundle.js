...
console.log("oci_actasreuniones.js running...");
...
var ajaxUrl = '/getDocs/oci/actasreuniones/' + client + '/' + ej;
console.log('...loading Oci Table ' + ajaxUrl);

$('#oci-actasreuniones').bootstrapTable({
  method: 'GET',
  url: ajaxUrl,
  columns: [{
    field: '_id',
...
var client = $('#comboClientes').val();
var ej = $('#comboEjercicios').val();
var ajaxUrl = '/getDocs/oci/memoria/' + client + '/' + ej;
console.log('...loading Oci memoria ' + ajaxUrl);

$('#oci-memoria').bootstrapTable({
  method: 'GET',
  url: ajaxUrl,
  columns: [{
    field: '_id',
...
var client = $('#comboClientes').val();
var ej = $('#comboEjercicios').val();
var ajaxUrl = '/getDocs/oci/personal/' + client + '/' + ej;
...
$('.btnAdd').on('click', function (e) {
  var collection = e.currentTarget.parentElement.parentElement.id.replace('_', '/').split('/')[0];
  var category = e.currentTarget.parentElement.parentElement.id.replace('_', '/').split('/')[1].toLowerCase();
  var client = $('#comboClientes').val();
  var clientId = comboClientes.options[comboClientes.selectedIndex].getAttribute('data-cliid');
  var exercice = $('#comboEjercicios').val();
...
function reloadOciTable(bTable) {
  var tName = $(bTable).attr('id');
  console.log('...reloading ' + tName);
  var client = Number($('#comboClientes').val());
  var ej = Number($('#comboEjercicios').val());
  var col = $(bTable).attr('id').replace('-', '/');
  var ajaxUrl = '/getDocs/' + col + '/' + client + '/' + ej;
  $(bTable).bootstrapTable('removeAll');
  $(bTable).bootstrapTable('refresh', { url: ajaxUrl });
}

function comboClientesChanged(param) {
  $('.myTable[id]').each(function (i, bTable) {
...
$('.myTable').on('click-cell.bs.table', function (field, value, row, $element) {
  if (value === 'nombreDocumento') {
    var fileToDownload = window.location.origin + '/getFile/' + $element.nombreFichero;
    var win = window.open(fileToDownload, '_blank');
    if (win) {
      win.focus();
    } else {
...
var operateEvents = {
  'click .edit': function clickEdit(e, value, row, index) {
    ...
    if (respuesta === true) {
      var cadena = String(JSON.stringify(row));
  ...
  'click .delete': function clickDelete(e, value, row, index) {
    e.preventDefault();
    var tableId = e.currentTarget.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.id.replace('_', '-');
    console.log('tableID = ' + tableId);
    var collection = e.currentTarget.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.id.replace('_', '-').split('-')[0];
    console.log('collection = ' + collection);
    var category = e.currentTarget.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.id.replace('_', '-').split('-')[1];
    console.log('category = ' + category);
    var respuesta = confirm('¿Realmente desea borrar el documento?');
    if (respuesta === true) {
      var cadena = String(JSON.stringify(row));
      // Doc = cadena.substring(cadena.indexOf('"id":')+5,cadena.indexOf('}'));
      // alert('Delete...');
